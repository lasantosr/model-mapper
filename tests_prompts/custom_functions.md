# Integration Test: `tests/custom_functions.rs`
...
Implement integration tests in [tests/custom_functions.rs](file:///home/ubuntu/projects/model-mapper/model-mapper/tests/custom_functions.rs) verifying custom mapping functions and expressions.

## Scope of Implementation

- Make edits to [tests/custom_functions.rs](file:///home/ubuntu/projects/model-mapper/model-mapper/tests/custom_functions.rs) ONLY.
...
- Reference vs Value trait dispatch: verify mapping functions taking references (like `String::len`) vs ownership (like `i32::abs`) to validate library `RefMapper`/`ValueMapper` traits.
- Cross-field expression scope: mapping expressions referencing multiple source fields (e.g. `into_with = format!("{},{}", field1, field2)`).
- Custom function name generation: verify the default snake-case function names generated when using `custom` without an explicit name (e.g. `from(custom)` / `into(custom)` with namespaced or simple type paths).

## References & Best Practices

- Refer to [with.rs](file:///home/ubuntu/projects/model-mapper/model-mapper/examples/with.rs) to understand custom mapping logic.
- Follow Rust integration testing best practices (use public API only).
- Feel free to implement any other related custom function tests you find useful.
