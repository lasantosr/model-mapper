# Integration Test: `tests/multiple_targets.rs`
...
Implement integration tests in [tests/multiple_targets.rs](file:///home/ubuntu/projects/model-mapper/model-mapper/tests/multiple_targets.rs) verifying mapping one source type to multiple target types.

## Scope of Implementation

- Make edits to [tests/multiple_targets.rs](file:///home/ubuntu/projects/model-mapper/model-mapper/tests/multiple_targets.rs) ONLY.
- Do NOT perform any git commits, branches, or external operations—only perform local file edits.

## Minimum Test Coverage Requirements

- Struct/enum deriving mappings for multiple targets using multiple `derive(...)` properties.
- Struct/enum deriving mappings for multiple targets using multiple separate `#[mapper(...)]` blocks.
- Target-specific overrides at variant/field levels using `#[mapper(when(ty = TargetType, ...))]`.
- Mixed attributes: default settings (e.g. `#[mapper(skip(default))]`) combined with `when`-specific overrides.

## References & Best Practices

- Refer to [multiple_derives.rs](file:///home/ubuntu/projects/model-mapper/model-mapper/examples/multiple_derives.rs) to see how multiple derives are written.
- Note that mapping a single source to multiple targets can be configured using either the `derive(...)` properties wrapper on a single attribute, or by declaring multiple separate `#[mapper(...)]` attribute blocks.
- Note that you can derive both directions (`from` and `into`) for a target within the same attribute block, or use multiple separate derive attributes / `derive(...)` blocks specifying one direction each.
- Follow Rust integration testing best practices (use public API only).
- Feel free to implement any other related multiple-target mapping tests you find useful.
