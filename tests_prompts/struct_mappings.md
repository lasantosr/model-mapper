# Integration Test: `tests/struct_mappings.rs`
...
Implement integration tests in [tests/struct_mappings.rs](file:///home/ubuntu/projects/model-mapper/model-mapper/tests/struct_mappings.rs) verifying struct mappings.

## Scope of Implementation

- Make edits to [tests/struct_mappings.rs](file:///home/ubuntu/projects/model-mapper/model-mapper/tests/struct_mappings.rs) ONLY.
- Do NOT perform any git commits, branches, or external operations—only perform local file edits.

## Minimum Test Coverage Requirements

- Named-field structs mapping directions (`from`, `into`, `try_from`, `try_into`).
- Tuple / unnamed-field structs mapping directions.
- Newtype structs mapping directions.
- Implicit `Into` widening (e.g. `i32` -> `i64`, `&str` -> `String`, `String` -> `PathBuf`).
- Same-type mapping (Self -> Self).
- Round-trip validation (Source -> Target -> Source).
- *Note:* Unit structs are unsupported by the macro and should not be tested here (they will be tested in compile-fail tests).

## References & Best Practices

- Refer to [exact_match.rs](file:///home/ubuntu/projects/model-mapper/model-mapper/examples/exact_match.rs) to understand simple exact struct mappings.
- Follow Rust integration testing best practices (use public API only).
- Ensure all test helper structs/enums derive common traits (`Debug`, `Clone`, `PartialEq`) as appropriate.
- Feel free to implement any other related struct mapping tests you find useful.
